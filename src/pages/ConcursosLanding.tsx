...
import { ArrowRight, Building2, FileText, MapPin, LogIn, LogOut, LayoutDashboard, Stethoscope, GraduationCap, Layers } from 'lucide-react';
      ...
      <main className="mx-auto max-w-[1200px] px-4 py-12 sm:px-6 sm:py-16 lg:px-10 xl:px-12">
        ...
        <div className="grid gap-5 sm:grid-cols-2">
          {concursos.map((c) => {
            ...
            const isInteg = c.tipo === 'integradora';
            const Icon = isInteg ? Stethoscope : Building2;
            return (
            <button
              key={c.slug}
            ...
            >
              <div className="mb-5 flex items-center gap-3">
                <div className="flex h-12 w-12 items-center justify-center rounded-2xl bg-gradient-to-br from-slate-500/10 to-slate-600/5 transition-transform group-hover:scale-110">
                  <Icon className={`h-6 w-6 ${ct.iconColor}`} />
                </div>
                <div>
                  <p className={`text-[10.5px] font-bold uppercase tracking-[0.18em] ${ct.textHighlight}`}>
                    {isInteg ? 'Afya · Medicina' : `Edital ${c.numeroEdital}`}
                  </p>
                  <h2 className="font-['Manrope'] text-2xl font-extrabold tracking-tight text-slate-950">
                    {isInteg ? 'Prova Integradora' : c.municipio}
                  </h2>
                </div>
              </div>
              ...
              <p className="mb-5 text-sm leading-relaxed text-slate-600">
                {isInteg ? (
                  <>Provas integradoras reais, comentadas questão a questão, organizadas por turma. Treine no formato exato da sua prova.</>
                ) : (
                ...
                {isInteg ? (
                  <>
                    <span className="inline-flex items-center gap-1.5">
                      ...
                      {c.provas?.length ?? 0} provas comentadas
                    </span>
                    <span className="inline-flex items-center gap-1.5">
                      <Layers className="h-4 w-4 text-amber-500" />
                      {c.turmas?.length ?? 0} turmas
                    </span>
                    <span className="inline-flex items-center gap-1.5">
              ...
              <div className={`mt-auto inline-flex h-11 items-center justify-center gap-2 rounded-xl bg-gradient-to-br ${ct.gradient} px-5 text-sm font-bold text-white shadow-[0_8px_24px_-8px_rgba(37,99,235,0.45)] transition-all group-hover:brightness-110 cai-sheen`}>
                {isInteg ? 'Acessar provas' : 'Estudar'}
